#########################################################################
# Name: QueueNotifier                                                      #
# Desc: Notifies you when queue pops #
# Author: Ninthwalker                                                   #
...
function QueueNotifier {
    ...
    :check Do {
        ...
        Get-Queue
        $queueAlert = (Get-Ocr $path\Queue_Img.png).Text
        if ($notifyOnDisconnect) {
            if (($queueAlert -like "*disconnected*")) {
                $disconnected = $True
            }
        }     
    }
    Until ($queueAlert -notlike "*Position in Queue*")
...
# Button methods
$button_start.Add_Click({QueueNotifier})
$button_stop.Add_Click({
    if (Test-Path $path\Queue_Img.png) {
        Remove-Item $path\Queue_Img.png -Force -Confirm:$False
    }
    $script:cancelLoop = $True
})
